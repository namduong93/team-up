...
export const AttendeesPageButtons: FC<AttendeesButtonsProps> = (
  ...
  const downloadCSV = async () => {
    ...
    const uniqueSizes = [...new Set(tShirtOptions
      ...
      .map(option => option.value.slice(1)) // Remove M/L prefix to get size
    )];
    ...
    csvContent += `Mens,${uniqueSizes.map(size => tshirtCounts[size].male).join(',')}\n`;
    ...
    .map((attendee) => {
      ...
      return {
        ...
        details: attendee.allergies 
          ? `${dietaryLabel || ''} ${attendee.allergies ? `(Allergies: ${attendee.allergies})` : ''}`
          : dietaryLabel || '',
  ...
  const downloadPDF = async () => {
    ...
    (doc as any).autoTable({
      startY: yPos,
      head: [
        ['Size:', ...uniqueSizes, 'Total'],
      ],
      body: [
        ['Male', ...uniqueSizes.map(size => tshirtCounts[size].male), 
          uniqueSizes.reduce((sum, size) => sum + tshirtCounts[size].male, 0)],
        ['Female', ...uniqueSizes.map(size => tshirtCounts[size].female),
          uniqueSizes.reduce((sum, size) => sum + tshirtCounts[size].female, 0)],
        ['Total', ...uniqueSizes.map(size => tshirtCounts[size].male + tshirtCounts[size].female),
          uniqueSizes.reduce((sum, size) => sum + tshirtCounts[size].male + tshirtCounts[size].female, 0)]
      ],
      theme: 'grid',
    ...
    if (dietaryRequirements.length > 0) {
      ...
      const dietaryTableData = Object.values(groupedDietary).flatMap(group => {

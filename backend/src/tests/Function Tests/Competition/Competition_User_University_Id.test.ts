...
describe('Get User University Id Function', () => {
  let user_db;
  let comp_db;
  let comp_staff_db;
  let comp_student_db;
  ...
  test('Success case: returns the university id of student', async () => {
    expect(await comp_db.getUserUniversityId(newStudent.userId)).toStrictEqual(1);
  });
});
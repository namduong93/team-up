...
describe('Coach Check Student Id Function', () => {
  let user_db;
  let comp_db;
  let comp_staff_db;
  let comp_student_db;
  ...
  beforeAll(async () => {
    ...
    const mockStudent: Student = {
      name: 'Maximillian Maverick',
      preferredName: 'X',
      email: 'newcontender12@gmail.com',
      password: 'testPassword',
      gender: 'Male',
      pronouns: 'He/Him',
      tshirtSize: 'L',
  ...
  test('Failure case: coach does not coach all of the user in the list', async () => {
    await expect(comp_staff_db.coachCheckIdsStudent(id, [newStudent.userId + 11], comp.competitionId)).rejects.toThrow('Coach is not coaching some of the students in the provided list');
  });

  test('Successfull case: does not throw an error', async () => {
    expect(await comp_staff_db.coachCheckIdsStudent(id, [newStudent.userId], comp.competitionId)).toStrictEqual(undefined);
  });
});
...
describe('Coach Check Student Id Function', () => {
  ...
  test('Failure case: coach does not coach all of the user in the list', async () => {
    await expect(comp_db.coachCheckIdsStudent(id, [newStudent.userId + 11], comp.competitionId)).rejects.toThrow("Coach is not coaching some of the students in the provided list")
  })

  test('Successfull case: does not throw an error', async () => {
    expect(await comp_db.coachCheckIdsStudent(id, [newStudent.userId], comp.competitionId)).toStrictEqual(undefined)
  })
})
...
describe('University Default Site Function', () => {
  let user_db;
  let comp_db;
  let uni_db;
  let comp_staff_db;
  ...
  beforeAll(async () => {
    ...
    const mockStudent: Student = {
      name: 'Maximillian Maverick',
      preferredName: 'X',
      email: 'newcontender23@gmail.com',
      password: 'testPassword',
      gender: 'Male',
      pronouns: 'He/Him',
      tshirtSize: 'L',
  ...
  test('Success case: returns detail of the default site', async () => {
    const siteInfo = await comp_db.competitionSites(comp.competitionId)
    expect(await comp_db.competitionUniversityDefaultSite(comp.competitionId, {
      id: 1,
      name: 'University of Melbourne'
    })).toStrictEqual({
      "id": siteInfo[0].id,
  ...
  test('Failure case: Site not found.', async () => {
    const failComp: Competition = {
      name: 'TestComp',
      teamSize: 5,
      createdDate: dateNow,
      earlyRegDeadline: earlyDate,
    ...
    const failCompInfo = await comp_staff_db.competitionSystemAdminCreate(id, failComp);

    await expect(comp_db.competitionUniversityDefaultSite(failCompInfo.competitionId, {
      id: 1,
      name: 'University of Melbourne'

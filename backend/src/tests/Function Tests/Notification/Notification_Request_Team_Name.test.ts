...
describe('Notification Request Team Name Function', () => {
  ...
  const member1: Student = {
    name: 'Test Member 1',
    preferredName: 'X',
    email: 'teamRequestName1@example.com',
    password: 'password1',
    gender: 'Male',
    pronouns: 'She/Her',
    tshirtSize: 'M',
  ...
  test('Success Case', async () => {
    ...
    await comp_staff_db.competitionRequestTeamNameChange(teamMate1.userId, compId, 'new name');
    await notif_db.notificationRequestTeamNameChange(teamId, compId);
    ...
    expect(notifications1).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: 'name',
          message: expect.stringContaining(`Team ${teamDetails.teamName} has requested to change its name to new name for competition ${mockCompetition.name}.`),
        }),
      ])
    );
  });

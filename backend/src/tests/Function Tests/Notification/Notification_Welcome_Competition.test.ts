...
describe('Notification Request Team Name Function', () => {
  ...
  const member1: Student = {
    name: 'Test Member 1',
    preferredName: 'X',
    email: 'teamWelcomeNotification@example.com',
    password: 'password1',
    gender: 'Male',
    pronouns: 'She/Her',
    tshirtSize: 'M',
  ...
  test('Success Case', async () => {
    ...
    await notif_db.notificationWelcomeToCompetition(teamMate1.userId, compId);
    ...
    expect(notifications1).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          message: expect.stringContaining(`Welcome to competition ${mockCompetition.name}.`),
        }),
      ])
    );
  });
